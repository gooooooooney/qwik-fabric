...
type FabricObject = fabric.BaseFabricObject
type Point = fabric.Point
type FabricCanvas = fabric.Canvas
type CanvasEvents = fabric.CanvasEvents
    ...
    private objectMoving({ target }: CanvasEvents['object:moving']) {
        ...
        const add = (group: any) => {
            const objects = group.getObjects().filter((obj) => {
                ...
                // if (fabric.util.isCollection(obj) && target.group && obj === target.group) {
                //     add(obj)
                //     return false
                // }
                return true
